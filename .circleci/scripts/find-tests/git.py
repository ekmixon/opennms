...
def get_parent_branch(repo_path):
    ...
    with open(os.path.join(repo_path, ".nightly"), "r") as nightly:
        for line in nightly:
            if match := re.match(r'parent_branch: (.*)$', line, re.M | re.I):
                parent_branch = match[1]
...
def get_changed_files(repo_path):
    ...
    print(f"Comparing: {current_branch} to: {parent_branch}")

    try:
        files_changed_in_commits = (
            subprocess.check_output(
                [
                    'git',
                    'diff',
                    '--name-only',
                    f'origin/{parent_branch}...HEAD',
                ]
            )
            .decode('utf-8')
            .split('\n')
